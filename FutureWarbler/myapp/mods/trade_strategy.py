...
from myapp.models import Historya
...
def FixedSizeStrategy(self, isBuy):
    if isBuy == True:
        ...
        # print(self.tobuy)
        # print(self.order)
        Historya.objects.create(member_id=self.userName, buy_qty=1, buy_mon=price,
                                buy_time=self.datas[0].datetime.date(0), futures_id=self.doData)
    else:
        self.order = self.sell()
        self.tosell = self.tosell + self.dataclose[0]
        # print(self.tobuy)
        # print(self.tosell)
        Historya.objects.create(member_id=self.userName, sell_qty=abs(self.broker.getposition(
            self.data).size), sell_mon=self.dataclose[0], sell_time=self.datas[0].datetime.date(0), futures_id=self.doData)
...
def FixedAmountStrategy(self, isBuy):
    if isBuy == True:
        ...
        self.order = self.buy(price=price)
        self.tobuy = self.tobuy + price
        Historya.objects.create(member_id=self.userName, buy_qty=count, buy_mon=price,
                                buy_time=self.datas[0].datetime.date(0), futures_id=self.doData)
    else:
        self.order = self.sell()
        self.tosell = self.tosell + self.dataclose[0]
