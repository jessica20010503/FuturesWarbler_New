...
class TechnicalIndicatorsImgFacade:
    ...
    def CreatLineDate(self,doType):
        if doType == "KD":
            kdDate = self.TechnicalIndicators(day_count=5, data=self.chart_data,doType=doType)
            line = (
            Line()
            ...
            .add_yaxis(
                ...
                linestyle_opts=opts.LineStyleOpts(width=3, opacity=0.5,color="#BF3434"),
            ...
            .add_yaxis(
                ...
                is_smooth=True,
                is_hover_animation=False,
                linestyle_opts=opts.LineStyleOpts(width=3, opacity=0.5,color="#262626"),
                label_opts=opts.LabelOpts(is_show=False),
            )
            )
            return line
            ...
            MacdDate = self.TechnicalIndicators(day_count=5, data=self.chart_data,doType=doType)
            line = (
            Line()
            .add_xaxis(xaxis_data=self.chart_data["categoryData"])
            .add_yaxis(
                ...
                is_smooth=True,
                is_hover_animation=False,
                linestyle_opts=opts.LineStyleOpts(width=3, opacity=0.5,color="#63A6A6"),
                label_opts=opts.LabelOpts(is_show=False),
            )
            .add_yaxis(
                series_name="MACD",
                ...
                is_smooth=True,
                is_hover_animation=False,
                linestyle_opts=opts.LineStyleOpts(width=3, opacity=0.5,color="#D97E6A"),
                label_opts=opts.LabelOpts(is_show=False),
            )
            )
            return line
            ...
            .add_yaxis(
                series_name=doType,
                #UseMA
                y_axis=self.TechnicalIndicators(day_count=5, data=self.chart_data,doType=doType),
                is_smooth=True,
                is_hover_animation=False,
    ...
    def draw_charts(self):
        ...
        grid_chart.add(
            kline,
            grid_opts=opts.GridOpts(pos_left="6%", pos_right="8%", height="20%"),
        )
        #宣告線圖1
        grid_chart.add(
            line,
...
# 計算週期
        ...
        df['DateTime'] =pd.to_datetime(df[self.futures+'_date'] + df[self.futures+'_time'], format='%Y-%m-%d%H:%M:%S')
        df[self.futures+'_date'] = pd.to_datetime(df[self.futures+'_date'])
        df[self.futures+'_time'] = pd.to_datetime(df[self.futures+'_time'])
        mask = (df[self.futures+'_date'] >= self.startTime) & (df[self.futures+'_date'] <= self.end)
